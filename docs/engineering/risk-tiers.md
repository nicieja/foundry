...
- **T2:** schema and migrations; public API; core domain logic; killing a bet and opening the next one; root toolchain files (`package.json`, `turbo.json`, `biome.json`, `tsconfig.base.json`, `bin/ci`, `bin/evidence-check`, `CLAUDE.md`); hooks in `.claude/settings.json`.
